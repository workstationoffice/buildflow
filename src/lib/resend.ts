...
export async function sendCheckInEmail(toEmails: string[], data: CheckInEmailData) {
  ...
  await getResend().emails.send({
    ...
    to: toEmails,
    subject: `[ConstructFlow] ${data.workerName} checked in at ${data.siteName}`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p style="color: #64748b; font-size: 12px; margin-top: 24px;">Sent by ConstructFlow</p>
      </div>
    `,
  });
}
...
export async function sendInviteEmail({
  ...
  await getResend().emails.send({
    from: process.env.RESEND_FROM_EMAIL!,
    to,
    subject: `You've been invited to join ${companyName} on ConstructFlow`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0f172a;">You're invited!</h2>
        <p>Hi ${name},</p>
        <p><strong>${inviterName}</strong> has invited you to join <strong>${companyName}</strong> on ConstructFlow.</p>
        ...
        <p style="color: #94a3b8; font-size: 12px; margin-top: 24px;">Sent by ConstructFlow</p>
      </div>
    `,
  });
}
